...
public class EgovLoginController {
	...
	public String actionLogout(HttpServletRequest request, ModelMap model) throws Exception {
		...
		((HttpServletRequest)request).getSession().setAttribute("isLocallyAuthenticated", null);
		...
		try {
			...
			EgovSessionCookieUtil.removeSessionAttribute(httpServletRequest, "SPRING_SECURITY_CONTEXT");

			EgovSessionCookieUtil.removeSessionAttribute(httpServletRequest, WoodCraftConstant.SessionAttribute.ALL_MENU_LIST);
			EgovSessionCookieUtil.removeSessionAttribute(httpServletRequest, WoodCraftConstant.SessionAttribute.MENU_LIST);
		} catch (Exception ex) {
			LOGGER.error(ex.getMessage());
		}
		//((HttpServletResponse)response).sendRedirect(((HttpServletRequest)request).getContextPath() + "/egov_security_logout");
		return "redirect:/index.do";
		//return ((HttpServletRequest)request).getContextPath() + "/egov_security_logout";
	}
