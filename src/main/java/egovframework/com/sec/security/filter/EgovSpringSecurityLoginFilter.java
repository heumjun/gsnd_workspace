		...
		if (EgovUserDetailsHelper.getAuthenticatedUser() == null || requestURL.contains(loginProcessURL)) {
			...
			if (isRemotelyAuthenticated != null && isRemotelyAuthenticated.equals("true")) {
				...
				if (requestURL.contains(loginProcessURL)) {
					...
					//비회원로그인 처리.
					LoginVO authVO = (LoginVO) session.getAttribute("authVO");

					String password;
					LoginVO loginVO = null;
					if (authVO == null || EgovStringUtil.isEmpty(authVO.getId())) {
...
//						dispatcher.forward(httpRequest, httpResponse);
							session.setAttribute("SPRING_SECURITY_LAST_EXCEPTION", "비밀번호는 8자 이상이거나 20자를 초과해서는 안됩니다.");
...
//						chain.doFilter(request, response);
					...
					try {
						...
						if (authVO == null || EgovStringUtil.isEmpty(authVO.getId())) {
							...
							LoginVO authVO2 = new LoginVO();
							String sDupInfo = (String) session.getAttribute("sDupInfo");
							authVO2.setIhidNum(sDupInfo);

							loginVO = loginService.actionAuthLogin(authVO2);
						}
						...
						if (loginVO != null && loginVO.getId() != null && !loginVO.getId().equals("")) {
							...
							// 비회원로그인의 SPRING_SECURITY_CONTEXT 가 생성되지 않아 강제 생성
							if (authVO != null && !EgovStringUtil.isEmpty(authVO.getId())) {
								...
								Authentication authentication = new UsernamePasswordAuthenticationToken(loginVO, "USER_PASSWORD", userdetail.getAuthorities());
							 ...
							 */
							EgovSessionCookieUtil.removeSessionAttribute(httpRequest, WoodCraftConstant.SessionAttribute.ALL_MENU_LIST);
							EgovSessionCookieUtil.removeSessionAttribute(httpRequest, WoodCraftConstant.SessionAttribute.MENU_LIST);
							...
							if (authVO != null && !EgovStringUtil.isEmpty(authVO.getId())) {
								...
								session.removeAttribute("sDupInfo");

								httpResponse.sendRedirect("/mokgong/authLogin.do");
								return;
							}
...
//							dispatcher.forward(httpRequest, httpResponse);
							...
							session.setAttribute("SPRING_SECURITY_LAST_EXCEPTION", "사용자 정보가 없거나 비밀번호가 다릅니다.");
							httpResponse.sendRedirect(loginURL);
...
//						dispatcher.forward(httpRequest, httpResponse);
//						chain.doFilter(request, response);
						session.setAttribute("SPRING_SECURITY_LAST_EXCEPTION", "로그인 중 오류가 발생하였습니다. 관리자에게 문의하십시오.");
						httpResponse.sendRedirect(loginURL);
