...
public class LoginController {
    ...
    public String loginSuccess(HttpServletRequest request) throws Exception {
        ...
        if (EgovStringUtil.nullConvert(request.getSession().getAttribute(HumanConstant.SessionAttribute.BEFORE_LOGIN_URL)).length() == 0) {
           redirectURL += "/index.do";
        } else {
            redirectURL += request.getSession().getAttribute(HumanConstant.SessionAttribute.BEFORE_LOGIN_URL).toString();
            request.getSession().removeAttribute(HumanConstant.SessionAttribute.BEFORE_LOGIN_URL);
        }
